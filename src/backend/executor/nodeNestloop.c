...
ExecNestLoop(PlanState *pstate)		// 实际上是迭代器函数，并不是一次执行完全部join操作
{
	...
	CHECK_FOR_INTERRUPTS();		// 检查是否有中断信号（不深究）
	...
	joinqual = node->js.joinqual;		// join条件
	otherqual = node->js.ps.qual;		// 其他条件(如where条件)
	...
	for (;;)
	{
		 ...
		 * if there are no more outer tuples, then the join is complete..
			...
			*/
			 ...
			 */
			foreach(lc, nl->nestParams)	// 将外表的一些参数传递给内表
			{
			...
			ExecReScan(innerPlan);	// 重新确定内表从哪里开始扫描
		}
			...
			*/
			if (!node->nl_MatchedOuter &&
				(node->js.jointype == JOIN_LEFT ||
				 node->js.jointype == JOIN_ANTI))	// 当：没找到匹配项，且是左连接或者反连接
			{
				/*
				...
				for(int i = 0; i < block.size; i++){
					...
					if(!block.isMatched[i]){
						econtext->ecxt_innertuple = node->nl_NullInnerTupleSlot;	// set to a empty tuple
						...
						if (otherqual == NULL || ExecQual(otherqual, econtext))
						{
							...
							return ExecProject(node->js.ps.ps_ProjInfo);	// 返回连接之后的tuple
						}
						else
